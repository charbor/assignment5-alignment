...
def grpo_minibatch_train_step(
        policy_log_probs: torch.Tensor,
        response_mask: Float[torch.Tensor, "batch seq"],
    ...
    loss_per_token, meta_data = compute_policy_gradient_loss(policy_log_probs=policy_log_probs,
                                                loss_type=loss_type,
    ...
    loss = masked_mean(loss_per_token, response_mask) / gradient_accumulation_steps
    loss.backward()
